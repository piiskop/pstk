...
class View
{
	...
	private static function findDifference($parameters)
	{
		...
		$partsOfTime = array();
		if ($difference->y > 0) {
			$partsOfTime[] = \psdtohtml\View::generatePartialString(
				array(
					'time' => $difference->y,
	...
	private static function buildCss()
	{
		...
		$tpl->loadTemplatefile('psdtohtml-template.css');
	...
	public static function buildView($parameters)
	{
		...
		$tpl->setVariable(
			array(
				...
				'STYLE' => \psdtohtml\View::buildCss(),
				'TIME' => \psdtohtml\View::findDifference(
					array(
