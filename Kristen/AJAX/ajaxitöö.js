...
function salvesta(){
	...
	if (foundby==="eesnimi" && bits[0]!= ""){
		...
		if (bits.length>1){
			pupils[currentIndex].lastname = bits.pop();
			pupils[currentIndex].firstname = bits.join([" "]);
			save.innerHTML= originalFname + " " + originalLname + " is now changed to " + pupils[currentIndex].firstname + " " + pupils[currentIndex].lastname;
		};
...
var makeRequestFirstname = function () {
	...
	if (input) {
		...
		if (!httpRequest) {
			console.log('258: Giving up :(Cannot create an XMLHTTP instance)');
			return false;
		}
		httpRequest.onreadystatechange = function(){
				if (httpRequest.readyState === XMLHttpRequest.DONE) {
					if (httpRequest.status === 200) {
						...
						document.getElementById("progebox").checked = false;
						otsingFirstname();
					} else {
		...
		httpRequest.open('GET', "http://localhost/pstk/Kristen/AJAX/ajaxitöö.php");
		httpRequest.send();
	}
}
var makeRequestLastname = function () {
	var input = document 
	if (input) {
		var httpRequest;
		httpRequest = new XMLHttpRequest();
...
var POSTajax = function() {
	...
	httpRequest.open('POST', 'http://localhost/pstk/Kristen/AJAX/ajaxitöö.php');
