...
class Abi	//kohtutäituri abi
{
	...
	{
		if ($owner->getIsPresent() && $owner->getAgrees()) //kui omanik on esindatud ja ta on nõus, siis ava lukk
		{
			$lockMaster = new LockMaster();
			$lockMaster->openLock($lock);
		}
		else if (!$owner->getIsPresent())	// kui omanikku ei ole, siis ..
		{
