...
(function (window, document, Math, JSON, Promise, console) {
...

    {
         ...
         * @type Number
         */
        var currentPupilId = {ID};
        
        /**
         * For saving PHP steps made for searching
         * 
         * @type Number
         */
        var steps = {STEPS};
...

    {
        ...
        /**
         * 
         * @param {String} stepsDone
         * @returns {undefined}
         */
        var searchResults = function (stepsDone) {

            document.getElementById('repeated').innerHTML =
                    "korduste arv: " + stepsDone;

            if (currentPupilId > 0) {
                document.getElementById('update-status').innerHTML =
                        "Leiti õpilane: " + pupils[currentPupilId - 1].name;

                document.getElementById("coding").checked =
                        pupils[currentPupilId - 1].coder;

                document.getElementById("name").value =
                        pupils[currentPupilId - 1].name;

                fillTable();
            } else {
                document.getElementById('update-status').innerHTML =
                        "Sellist õpilast pole";
            }
        };
    ...
    {
        sort(pupils, defaultProperty).forEach(addId);
        if (steps) {//php GET search was active
            searchResults(steps.toString());
            steps = 0;
        }
    }//MAININIT

    {   //run tests by adding parameter runAllTESTS to url
        if (window.location.href.indexOf('runAllTESTS') > -1) {
            ...
            var jsTests = new Promise(
                    ...
                    /**
                     * BE testing get results by AJAX
                     * 
                     * @type app_L17.Promise
                     */
